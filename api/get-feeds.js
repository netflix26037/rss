export default async function handler(req, res) {
    ...
    try {
        ...
        if (filteredSources.length === 0 && !requestedUrl) {
            // قائمة احتياطية في حال تعذر قراءة feed.json
            filteredSources = [
        ...
        for (const source of filteredSources.slice(0, 6)) {
            try {
                ...
                if (rData.status === 'ok' && rData.items) {
                    for (const item of rData.items.slice(0, 5)) {
                        const cleanDesc = item.description ? item.description.replace(/<[^>]*>?/gm, '').trim().substring(0, 200) : '';
