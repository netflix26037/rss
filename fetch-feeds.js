...
// دالة الترجمة إلى العربية باستخدام Google Translate
async function translateText(text) {
  ...
  try {
    ...
    // في حال فشل الترجمة يتم إرجاع النص الأصلي دون إيقاف السكريبت
    return text;
  }
}
    ...
    for (const source of sources) {
      ...
      try {
        ...
        if (feed && feed.items && feed.items.length > 0) {
          // جلب أول 5 مقالات فقط من كل مصدر لتسريع الترجمة وتفادي الحظر
          ...
          console.log(`✓ [نجاح وترجمة] تم جلب ${items.length} مقال من: ${source.name}`);
        } else {
