...
async function translateViaLibreTranslate(cleanText) {
    for (const endpoint of LIBRETRANSLATE_MIRRORS) {
        try {
            ...
            console.log(`فشل LibreTranslate (${endpoint}): ${e.message}`);
...
async function translateText(text) {
    ...
    // المحاولة 1: LibreTranslate (يجرب كل المرايا بالترتيب)
    let result = await translateViaLibreTranslate(cleanText);
    if (result) return result;

    // المحاولة 2: MyMemory كخط دفاع أخير
    result = await translateViaMyMemory(cleanText);
    if (result) return result;
